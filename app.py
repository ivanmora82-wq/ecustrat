...
# --- 1. CONEXIÓN BLINDADA (SOLUCIÓN PANTALLA ROJA) ---
def conectar_seguro():
    scope = ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive"]
    # Clave privada directa con limpieza de saltos de línea
    ...
    # TRIPLE LIMPIEZA: Asegura que Streamlit lea la llave correctamente
    pk = pk_raw.replace('\\n', '\n').strip()
...
def leer_hoja(nombre):
    try:
        ws = conectar_seguro().worksheet(nombre)
        df = pd.DataFrame(ws.get_all_records())
        if not df.empty:
            df['Monto'] = pd.to_numeric(df['Monto'], errors='coerce').fillna(0)
            df['Fecha'] = pd.to_datetime(df['Fecha']).dt.date
            if 'Estado' not in df.columns: df['Estado'] = 'PENDIENTE'
            df['idx_real'] = df.index + 2
        return df
    except Exception as e:
        return pd.DataFrame(columns=['Fecha', 'Sede', 'Concepto', 'Monto', 'Estado', 'idx_real'])

# --- 2. CONFIGURACIÓN ---
st.set_page_config(page_title="EMI MASTER V170", layout="wide")
hoy = date.today()
...
with st.sidebar:
    ...
    # Carga de datos
    df_v = leer_hoja("Ventas"); df_f = leer_hoja("Fijos"); df_p = leer_hoja("Proveedores"); df_c = leer_hoja("Cobros")
    
    # Cálculo de Arrastre
    v_h = df_v[df_v['Fecha'] < primer_dia_mes]['Monto'].sum() if not df_v.empty else 0
    c_h = df_c[(df_c['Fecha'] < primer_dia_mes) & (df_c['Estado'] == 'COBRADO')]['Monto'].sum() if not df_c.empty else 0
    g_h = (df_f[df_f['Estado'] == 'PAGADO'].Monto.sum() if not df_f.empty else 0)
    ...
    st.metric("BALANCE NETO REAL", f"$ {round(b_ini + c_ini + saldo_arrastre + v_mes, 2)}")

# --- 4. RENDERIZADO POR PESTAÑAS ---
tabs = st.tabs(["💰 VENTAS", "🏢 GASTOS", "🚛 PROVEEDORES", "📞 COBROS"])

def render_modulo_seguro(hoja, alias, label, txt_ok):
    df_raw = leer_hoja(hoja)
    df = df_raw[(df_raw['Fecha'] >= primer_dia_mes) | (df_raw['Estado'] == 'PENDIENTE')] if not df_raw.empty else pd.DataFrame()
    ...
    with st.expander(f"➕ Registrar {label}"):
        with st.form(f"form_{alias}", clear_on_submit=True):
            ...
            if st.form_submit_button("GRABAR"):
                try:
                    conectar_seguro().worksheet(hoja).append_row([str(f_r), sede_act, det, m_r, "PENDIENTE"])
                    st.success("Grabado correctamente"); st.rerun()
    ...
    if not df.empty:
        for _, row in df.iterrows():
            ...
            # Selector de acción
            op = c3.selectbox("Acción", ["...", f"Confirmar {txt_ok}", "Borrar"], key=f"ac_{alias}_{row['idx_real']}")
            if op == f"Confirmar {txt_ok}":
                conectar_seguro().worksheet(hoja).update_cell(row['idx_real'], 5, txt_ok); st.rerun()
            elif op == "Borrar":
                conectar_seguro().worksheet(hoja).delete_rows(row['idx_real']); st.rerun()
...
with tabs[0]: # VENTAS
    ...
    with st.form("fv"):
        ...
        if st.form_submit_button("GUARDAR VENTA"):
            conectar_seguro().worksheet("Ventas").append_row([str(f_v), sede_act, "Venta Diaria", m_v, "PAGADO"])
            st.rerun()
    if not df_v_mes.empty:
        for _, row in df_v_mes.iterrows():
            c1, c2, c3 = st.columns([6, 2, 2])
            c1.write(f"📅 {row['Fecha']} | Venta"); c2.write(f"**${row['Monto']}**")
            if c3.button("🗑️ Borrar", key=f"v_del_{row['idx_real']}"):
                conectar_seguro().worksheet("Ventas").delete_rows(row['idx_real']); st.rerun()

with tabs[1]: render_modulo_seguro("Fijos", "f", "Gasto", "PAGADO")
with tabs[2]: render_modulo_seguro("Proveedores", "p", "Proveedor", "PAGADO")
with tabs[3]: render_modulo_seguro("Cobros", "c", "Cobro", "COBRADO")
