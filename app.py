...
def conectar():
    ...
    # Formato de llave corregido para evitar errores de padding en Streamlit Cloud
...
st.set_page_config(page_title="EMI MASTER V140", layout="wide")
hoy = date.today()
...
# --- 3. BARRA LATERAL (SUMATORIAS Y BALANCE) ---
with st.sidebar:
...
def render_modulo(hoja, alias, label, txt_ok):
    ...
    if not df.empty:
        for _, row in df.iterrows():
            ...
            if c4.button("🗑️", key=f"del_{alias}_{row['idx']}"):
...
with tabs[0]: # VENTAS
    ...
    if not df_v_mes.empty:
        for _, row in df_v_mes.iterrows():
            ...
            if c3.button("🗑️", key=f"v_del_{row['idx']}"):
