...
def conectar():
    ...
    # Formato de llave corregido para evitar errores de padding
...
st.set_page_config(page_title="EMI MASTER V160", layout="wide")
hoy = date.today()
...
def render_modulo_compacto(hoja, alias, label, txt_ok):
    ...
    if not df.empty:
        for _, row in df.iterrows():
            ...
            # Selector de acción único para evitar lentitud
            accion = c3.selectbox("Acción", ["...", f"Confirmar {txt_ok}", "Revertir", "Borrar"], key=f"act_{alias}_{row['idx']}")
            
            if accion == f"Confirmar {txt_ok}":
...
with tabs[0]: # VENTAS
    ...
    if not df_v_mes.empty:
        for _, row in df_v_mes.iterrows():
            ...
            if c3.button("🗑️ Borrar", key=f"v_del_{row['idx']}"):
                conectar().worksheet("Ventas").delete_rows(row['idx']); st.rerun()

with tabs[1]: render_modulo_compacto("Fijos", "f", "Gasto Fijo", "PAGADO")
