...
st.markdown("""
    ...
    .stButton>button { width: 100%; border-radius: 5px; height: 32px; font-weight: bold; }
    </style>
...
with st.sidebar:
    ...
    df_v = leer_hoja("Ventas"); df_f = leer_hoja("Fijos"); df_h = leer_hoja("Hormiga")
    df_p = leer_hoja("Proveedores"); df_c = leer_hoja("Cobros")
    ...
    st.metric("BALANCE NETO ACTUAL", f"$ {round(b_ini + c_ini + arrastre + v_mes + c_mes - f_mes - p_mes, 2)}")
...
def render_modulo(hoja, alias, label, icn, est_ok):
    ...
    with st.expander(f"➕ Registrar {label}"):
        with st.form(f"f_{alias}", clear_on_submit=True):
            f_r = st.date_input("Fecha", hoy); nom = st.text_input("Nombre"); m_r = st.number_input("Monto $", min_value=0.0)
            if st.form_submit_button("GRABAR"):
                conectar_base().worksheet(hoja).append_row([str(f_r), sede_act, nom, m_r, "PENDIENTE"])
                st.rerun()
    ...
    if not df.empty:
        for _, row in df.iterrows():
            ...
            if row['Estado'] == "PENDIENTE":
                if c3.button(icn, key=f"ok_{alias}_{row['Fila_ID']}"):
                    conectar_base().worksheet(hoja).update_cell(row['Fila_ID'], 5, est_ok); st.rerun()
            else:
                if c3.button("🔄", key=f"rev_{alias}_{row['Fila_ID']}"):
                    conectar_base().worksheet(hoja).update_cell(row['Fila_ID'], 5, "PENDIENTE"); st.rerun()
            
            c4.button("📝", key=f"ed_{alias}_{row['Fila_ID']}")
            if c5.button("🗑️", key=f"del_{alias}_{row['Fila_ID']}"):
                conectar_base().worksheet(hoja).delete_rows(row['Fila_ID']); st.rerun()

with tabs[0]: # VENTAS RECUPERADAS
    ...
    with st.form("fv"):
        f_v = st.date_input("Fecha", hoy); m_v = st.number_input("Monto Venta $", min_value=0.0)
    ...
    for _, row in df_v_mes.iterrows():
        ...
        c3.button("📝", key=f"edv_{row['Fila_ID']}")
        if c4.button("🗑️", key=f"delv_{row['Fila_ID']}"):
            conectar_base().worksheet("Ventas").delete_rows(row['Fila_ID']); st.rerun()
