...
with t_caja:
    ...
    with st.form("cierre_caja"):
        ...
        enviar_caja = st.form_submit_button("🚀 Finalizar Día")
        if enviar_caja:
...
with t_hormiga:
    ...
    with st.form("form_hormiga", clear_on_submit=True):
        ...
        añadir_h = st.form_submit_button("➕ Añadir Gasto")
        if añadir_h and monto_h > 0:
...
with t_prov:
    ...
    with st.form("form_prov", clear_on_submit=True):
        ...
        añadir_p = st.form_submit_button("Registrar Proveedor")
        if añadir_p and p_nom and p_val > 0:
    ...
    for i, p in enumerate(st.session_state.db_proveedores):
        if p['Estado'] == "Pendiente":
            ...
            if c_p2.button(f"Pagar", key=f"btnp_{i}"):
...
with t_cobros:
    ...
    with st.form("form_cobros", clear_on_submit=True):
        ...
        añadir_c = st.form_submit_button("Registrar Cobro")
        if añadir_c and c_cli and c_val > 0:
...
with t_rep:
    if not is_premium:
        st.error("🔒 Función Premium Bloqueada")
    else:
        st.success("Acceso Premium")
        if st.session_state.db_hormiga:
            df_rep = pd.DataFrame(st.session_state.db_hormiga)
            fig_rep = px.pie(df_rep, names="Concepto", values="Monto", title="Distribución de Gastos")
