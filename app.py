import streamlit as st
import pandas as pd
import gspread
from oauth2client.service_account import ServiceAccountCredentials
from datetime import date

# --- 1. CONEXIÓN OPTIMIZADA (MENOS LLAMADAS API) ---
@st.cache_resource(ttl=300)
def conectar_seguro():
    scope = ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive"]
    creds_dict = {
        "type": "service_account",
        "project_id": "fabled-ranger-480412-b9",
        "private_key_id": "5be77e02ce33b4b12f69dfdda644de61e5ff3d54",
        "private_key": "-----BEGIN PRIVATE KEY-----\\ CJvjiMjDqtfF9jpLqb1ViQ7jd0dZ8LeX7qOd791z/7Otn9LydcmhSjlf+HlvDXMo\nKYtFpUmSzjMJNrEIz51jmUNuSZJE3KiZnDC7VrQUkwb5iFyM90jp0g5ibDIyuH6Z\ \\n-----END PRIVATE KEY-----\n",
        "client_email": "emi-database@fabled-ranger-480412-b9.iam.gserviceaccount.com",
        "client_id": "102764735047338306868"
    }
    return gspread.authorize(ServiceAccountCredentials.from_json_keyfile_dict(creds_dict, scope)).open("EMI_DATA_PRO")

def fetch(hoja):
    try:
        df = pd.DataFrame(conectar_seguro().worksheet(hoja).get_all_records())
        if not df.empty:
            df['Monto'] = pd.to_numeric(df['Monto'], errors='coerce').fillna(0)
            df['Fecha'] = pd.to_datetime(df['Fecha']).dt.date
            df['ID'] = df.index + 2
        return df
    except: return pd.DataFrame()

# --- 2. LOGICA DE TIEMPO ---
hoy = date.today()
primer_dia = hoy.replace(day=1)

# --- 3. DISEÑO ---
st.set_page_config(page_title="EMI MASTER PRO", layout="wide")
st.markdown("""
    <style>
    [data-testid="stSidebar"] { background-color: #1c2e4a !important; }
    [data-testid="stSidebar"] label { color: #FFD700 !important; font-weight: bold; }
    .stMetric { background-color: #d4af37 !important; color: #1c2e4a !important; border-radius: 8px; }
    .sum-box { background-color: #1c2e4a; color: #FFD700; padding: 10px; border-radius: 5px; text-align: center; border: 1px solid #FFD700; font-weight: bold; margin-bottom: 10px; }
    .stButton>button { width: 100%; border-radius: 4px; height: 32px; font-weight: bold; }
    </style>
    """, unsafe_allow_html=True)

with st.sidebar:
    st.markdown("<h1 style='text-align: center; color: #FFD700;'>🛡️ EMI MASTER</h1>", unsafe_allow_html=True)
    sede = st.selectbox("📍 PUNTO DE VENTA", ["Matriz", "Sucursal 1", "Sucursal 2"])
    banco = st.number_input("🏦 SALDO BANCO", value=0.0)
    caja = st.number_input("💵 SALDO CAJA", value=0.0)
    
    # SALDO DE ARRASTRE (Optimizado para no causar errores)
    df_v = fetch("Ventas"); df_f = fetch("Fijos"); df_p = fetch("Proveedores"); df_c = fetch("Cobros"); df_h = fetch("Hormiga")
    
    def calc_arrastre():
        v = df_v[df_v['Fecha'] < primer_dia]['Monto'].sum() if not df_v.empty else 0
        c = df_c[(df_c['Fecha'] < primer_dia) & (df_c['Estado'] == 'COBRADO')]['Monto'].sum() if not df_c.empty else 0
        g = (df_f[df_f['Estado'] == 'PAGADO']['Monto'].sum() + df_p[df_p['Estado'] == 'PAGADO']['Monto'].sum() + df_h['Monto'].sum())
        return v + c - g

    saldo_arrastre = calc_arrastre()
    st.info(f"💾 **Saldo al {primer_dia}:**\n$ {saldo_arrastre}")

    v_mes = df_v[df_v['Fecha'] >= primer_dia]['Monto'].sum() if not df_v.empty else 0
    c_mes = df_c[(df_c['Fecha'] >= primer_dia) & (df_c['Estado'] == 'COBRADO')]['Monto'].sum() if not df_c.empty else 0
    f_mes = df_f[(df_f['Fecha'] >= primer_dia) & (df_f['Estado'] == 'PAGADO')]['Monto'].sum() if not df_f.empty else 0
    p_mes = df_p[(df_p['Fecha'] >= primer_dia) & (df_p['Estado'] == 'PAGADO')]['Monto'].sum() if not df_p.empty else 0

    st.metric("BALANCE NETO ACTUAL", f"$ {round(banco + caja + saldo_arrastre + v_mes + c_mes - f_mes - p_mes, 2)}")

# --- 4. RENDERIZADO ---
tabs = st.tabs(["💰 VENTAS", "🏢 FIJOS", "🐜 HORMIGA", "🚛 PROV", "📞 COBROS", "📊 REPORTES"])

def render_linea(hoja, alias, label, icn, txt_ok):
    df = fetch(hoja)
    df_v = df[(df['Fecha'] >= primer_dia) | (df['Estado'] == 'PENDIENTE')] if not df.empty else pd.DataFrame()
    
    st.markdown(f"<div class='sum-box'>TOTAL {hoja.upper()}: $ {df_v['Monto'].sum() if not df_v.empty else 0}</div>", unsafe_allow_html=True)
    
    with st.expander(f"➕ Registrar {label}"):
        with st.form(f"f_{alias}"):
            f_r = st.date_input("Fecha", hoy); nom = st.text_input(f"Nombre {label}"); m_r = st.number_input("Monto", min_value=0.0)
            if st.form_submit_button("GRABAR"):
                conectar_seguro().worksheet(hoja).append_row([str(f_r), sede, nom, m_r, "PENDIENTE"])
                st.rerun()

    for _, row in df_v.iterrows():
        c1, c2, c3, c4, c5 = st.columns([4, 2, 1, 1, 1])
        c1.write(f"{row['Fecha']} | {row['Concepto']}")
        c2.write(f"**${row['Monto']}**")
        
        # BOTONES CON IDENTIFICACIÓN CLARA
        if row['Estado'] == "PENDIENTE":
            if c3.button(icn, key=f"ok_{alias}_{row['ID']}", help=f"Marcar {txt_ok}"):
                conectar_seguro().worksheet(hoja).update_cell(row['ID'], 5, txt_ok); st.rerun()
        else:
            if c3.button("🔄", key=f"rev_{alias}_{row['ID']}", help="Revertir"):
                conectar_seguro().worksheet(hoja).update_cell(row['ID'], 5, "PENDIENTE"); st.rerun()
        
        c4.button("📝", key=f"ed_{alias}_{row['ID']}")
        if c5.button("🗑️", key=f"del_{alias}_{row['ID']}"):
            conectar_seguro().worksheet(hoja).delete_rows(row['ID']); st.rerun()

with tabs[0]: # VENTAS CON CONTROL
    df_v_mes = df_v[df_v['Fecha'] >= primer_dia]
    st.markdown(f"<div class='sum-box'>TOTAL VENTAS MES: $ {df_v_mes['Monto'].sum() if not df_v_mes.empty else 0}</div>", unsafe_allow_html=True)
    with st.form("fv"):
        f_v = st.date_input("Fecha", hoy); m_v = st.number_input("Valor $", min_value=0.0)
        if st.form_submit_button("GRABAR VENTA"):
            conectar_seguro().worksheet("Ventas").append_row([str(f_v), sede, "Venta", m_v, "PAGADO"])
            st.rerun()
    for _, row in df_v_mes.iterrows():
        c1, c2, c3, c4 = st.columns([5, 2, 1, 1])
        c1.write(f"{row['Fecha']} | Venta"); c2.write(f"**${row['Monto']}**")
        c3.button("📝", key=f"ved_{row['ID']}"); 
        if c4.button("🗑️", key=f"vdel_{row['ID']}"):
            conectar_seguro().worksheet("Ventas").delete_rows(row['ID']); st.rerun()

with tabs[1]: render_linea("Fijos", "f", "Gasto Fijo", "✅", "PAGADO")
with tabs[3]: render_linea("Proveedores", "p", "Proveedor", "✅", "PAGADO")
with tabs[4]: render_linea("Cobros", "c", "Cuenta Cobro", "✅", "COBRADO")

with tabs[5]:
    st.header("📊 Reportes")
    rep = st.selectbox("Análisis:", ["Máximo Proveedor", "Diferencia Ventas vs Gastos"])
    c1, c2 = st.columns(2)
    sd = c1.date_input("Desde", primer_dia); ed = c2.date_input("Hasta", hoy)
