...
def leer_datos_seguros(nombre_hoja):
    try:
        ws = conectar_base().worksheet(nombre_hoja)
        df = pd.DataFrame(ws.get_all_records())
        # BLINDAJE: Si la hoja está vacía o faltan columnas, las creamos para evitar errores
        columnas_necesarias = ['Fecha', 'Sede', 'Concepto', 'Monto', 'Estado']
        ...
        return pd.DataFrame(columns=['Fecha', 'Sede', 'Concepto', 'Monto', 'Estado', 'Fila'])
...
st.markdown("""
    ...
    .stMetric { background-color: #d4af37 !important; color: #1c2e4a !important; border-radius: 10px; padding: 10px; }
...
# --- 3. BARRA LATERAL (ARRASTRE DE SALDOS) ---
with st.sidebar:
    ...
    df_v = leer_datos_seguros("Ventas")
    df_f = leer_datos_seguros("Fijos")
    df_h = leer_datos_seguros("Hormiga")
    df_p = leer_datos_seguros("Proveedores")
    df_c = leer_datos_seguros("Cobros")
    ...
    def calcular_arrastre():
        ...
        f = df_f[(df_f['Fecha'] < primer_dia) & (df_f['Estado'] == 'PAGADO')]['Monto'].sum() if not df_f.empty else 0
        p = df_p[(df_p['Fecha'] < primer_dia) & (df_p['Estado'] == 'PAGADO')]['Monto'].sum() if not df_p.empty else 0
    ...
    # Totales mes actual para el Balance Neto
    v_m = df_v[df_v['Fecha'] >= primer_dia]['Monto'].sum() if not df_v.empty else 0
    c_m = df_c[(df_c['Fecha'] >= primer_dia) & (df_c['Estado'] == 'COBRADO')]['Monto'].sum() if not df_c.empty else 0
    f_m = df_f[(df_f['Fecha'] >= primer_dia) & (df_f['Estado'] == 'PAGADO')]['Monto'].sum() if not df_f.empty else 0
    p_m = df_p[(df_p['Fecha'] >= primer_dia) & (df_p['Estado'] == 'PAGADO')]['Monto'].sum() if not df_p.empty else 0

    st.metric("BALANCE NETO ACTUAL", f"$ {round(b_ini + c_ini + arrastre + v_m + c_m - f_m - p_m, 2)}")

# --- 4. PESTAÑAS Y RENDERIZADO ---
tabs = st.tabs(["💰 VENTAS", "🏢 FIJOS", "🐜 HORMIGA", "🚛 PROVEEDORES", "📞 COBROS", "📊 REPORTES"])

def render_fila(hoja, alias, label, icn, est_ok):
    df_all = leer_datos_seguros(hoja)
    ...
    with st.expander(f"➕ Registrar {label}"):
        with st.form(f"form_{alias}", clear_on_submit=True):
            ...
            if st.form_submit_button("GRABAR"):
                conectar_base().worksheet(hoja).append_row([str(f_r), sede_act, det, m_r, "PENDIENTE"])
                st.rerun()
    ...
    if not df.empty:
        for _, row in df.iterrows():
            ...
            if row['Estado'] == "PENDIENTE":
                if c3.button("✅", key=f"ok_{alias}_{row['Fila']}"):
                    conectar_base().worksheet(hoja).update_cell(row['Fila'], 5, est_ok); st.rerun()
            else:
                if c3.button("🔄", key=f"rev_{alias}_{row['Fila']}"):
                    conectar_base().worksheet(hoja).update_cell(row['Fila'], 5, "PENDIENTE"); st.rerun()
            c4.button("📝", key=f"ed_{alias}_{row['Fila']}")
            if c5.button("🗑️", key=f"del_{alias}_{row['Fila']}"):
                conectar_base().worksheet(hoja).delete_rows(row['Fila']); st.rerun()

with tabs[0]: # VENTAS CON CONTROL TOTAL
    ...
    with st.form("fv"):
        ...
        if st.form_submit_button("GUARDAR VENTA"):
            conectar_base().worksheet("Ventas").append_row([str(f_v), sede_act, "Venta Diaria", m_v, "PAGADO"])
            st.rerun()
    for _, row in df_v_mes.iterrows():
        c1, c2, c3, c4 = st.columns([5, 2, 1, 1])
        c1.write(f"{row['Fecha']} | Venta"); c2.write(f"**${row['Monto']}**")
        c3.button("📝", key=f"edv_{row['Fila']}")
        if c4.button("🗑️", key=f"delv_{row['Fila']}"):
            conectar_base().worksheet("Ventas").delete_rows(row['Fila']); st.rerun()

with tabs[1]: render_fila("Fijos", "f", "Gasto Fijo", "✅", "PAGADO")
with tabs[2]: render_fila("Hormiga", "h", "Gasto Hormiga", "✅", "PAGADO")
with tabs[3]: render_fila("Proveedores", "p", "Proveedor", "✅", "PAGADO")
with tabs[4]: render_modulo_final = render_fila("Cobros", "c", "Cuenta Cobro", "✅", "COBRADO")
