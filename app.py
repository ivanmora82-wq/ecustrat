import streamlit as st
import pandas as pd
import gspread
from oauth2client.service_account import ServiceAccountCredentials
from datetime import date

# --- 1. CONEXIÓN BLINDADA (SOLUCIÓN DEFINITIVA A PANTALLA ROJA) ---
def conectar_seguro():
    scope = ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive"]
    
    # Clave privada directa
    pk_raw = "-----BEGIN PRIVATE KEY-----\\\ nx/ggxcwRkCo9duUzkAndSS+q4326v6llE7PNLBONzksP4DJntG1saa0xZISSqueX\nKYPHsPn0nlseGFR1yNFXZg32FsIlKeLsCkIgrwtge7Qjccc7F8gOO7faV6strfq+\nKFggcG+4j6j1Xu1LpRniQy4=\n-----END PRIVATE KEY-----\n"
    
    # LIMPIEZA AUTOMÁTICA DE FORMATO
    pk = pk_raw.replace('\\n', '\n').strip()
    
    creds_dict = {
        "type": "service_account",
        "project_id": "fabled-ranger-480412-b9",
        "private_key_id": "5be77e02ce33b4b12f69dfdda644de61e5ff3d54",
        "private_key": pk,
        "client_email": "emi-database@fabled-ranger-480412-b9.iam.gserviceaccount.com",
        "client_id": "102764735047338306868"
    }
    creds = ServiceAccountCredentials.from_json_keyfile_dict(creds_dict, scope)
    return gspread.authorize(creds).open("EMI_DATA_PRO")

def leer_hoja(nombre):
    try:
        ws = conectar_seguro().worksheet(nombre)
        df = pd.DataFrame(ws.get_all_records())
        if not df.empty:
            df['Monto'] = pd.to_numeric(df['Monto'], errors='coerce').fillna(0)
            df['Fecha'] = pd.to_datetime(df['Fecha']).dt.date
            if 'Estado' not in df.columns: df['Estado'] = 'PENDIENTE'
            df['row_idx'] = df.index + 2
        return df
    except:
        return pd.DataFrame(columns=['Fecha', 'Sede', 'Concepto', 'Monto', 'Estado', 'row_idx'])

# --- 2. CONFIGURACIÓN ---
st.set_page_config(page_title="EMI MASTER V180", layout="wide")
hoy = date.today()
primer_dia_mes = hoy.replace(day=1)

st.markdown("""
    <style>
    [data-testid="stSidebar"] { background-color: #1c2e4a !important; }
    [data-testid="stSidebar"] label { color: #FFD700 !important; font-weight: bold; }
    .stMetric { background-color: #d4af37 !important; color: #1c2e4a !important; border-radius: 8px; }
    .sum-box { background-color: #1c2e4a; color: #FFD700; padding: 10px; border-radius: 8px; text-align: center; border: 1px solid #FFD700; font-weight: bold; margin-bottom: 15px; }
    .stButton>button { width: 100%; border-radius: 5px; height: 35px; font-weight: bold; }
    </style>
    """, unsafe_allow_html=True)

# --- 3. SIDEBAR (BALANCE TOTAL RECUPERADO) ---
with st.sidebar:
    st.markdown("<h1 style='text-align: center; color: #FFD700;'>🛡️ EMI MASTER</h1>", unsafe_allow_html=True)
    sede_act = st.selectbox("📍 PUNTO DE VENTA", ["Matriz", "Sucursal 1", "Sucursal 2"])
    b_ini = st.number_input("🏦 SALDO BANCO", value=0.0)
    c_ini = st.number_input("💵 SALDO CAJA", value=0.0)
    
    # Datos para cálculos
    df_v = leer_hoja("Ventas"); df_f = leer_hoja("Fijos"); df_p = leer_hoja("Proveedores"); df_c = leer_hoja("Cobros")
    
    def calc_arrastre():
        v = df_v[df_v['Fecha'] < primer_dia_mes]['Monto'].sum() if not df_v.empty else 0
        c = df_c[(df_c['Fecha'] < primer_dia_mes) & (df_c['Estado'] == 'COBRADO')]['Monto'].sum() if not df_c.empty else 0
        f = (df_f[(df_f['Estado'] == 'PAGADO') & (df_f['Fecha'] < primer_dia_mes)].Monto.sum() if not df_f.empty else 0)
        return v + c - f

    arrastre = calc_arrastre()
    st.info(f"💾 **Saldo al {primer_dia_mes}:**\n$ {arrastre}")
    
    v_mes = df_v[df_v['Fecha'] >= primer_dia_mes]['Monto'].sum() if not df_v.empty else 0
    st.metric("BALANCE NETO ACTUAL", f"$ {round(b_ini + c_ini + arrastre + v_mes, 2)}")

# --- 4. RENDERIZADO POR PESTAÑAS ---
tabs = st.tabs(["💰 VENTAS", "🏢 GASTOS", "🚛 PROVEEDORES", "📞 COBROS"])

def render_modulo(hoja, alias, label, icn, est_ok):
    df_raw = leer_hoja(hoja)
    df = df_raw[(df_raw['Fecha'] >= primer_dia_mes) | (df_raw['Estado'] == 'PENDIENTE')] if not df_raw.empty else pd.DataFrame()
    st.markdown(f"<div class='sum-box'>TOTAL {hoja.upper()}: $ {df['Monto'].sum() if not df.empty else 0}</div>", unsafe_allow_html=True)
    
    with st.expander(f"➕ Registrar {label}"):
        with st.form(f"form_{alias}", clear_on_submit=True):
            f_r = st.date_input("Fecha", hoy); det = st.text_input("Detalle"); m_r = st.number_input("Monto", min_value=0.0)
            if st.form_submit_button("GRABAR"):
                conectar_seguro().worksheet(hoja).append_row([str(f_r), sede_act, det, m_r, "PENDIENTE"])
                st.rerun()

    if not df.empty:
        for _, row in df.iterrows():
            c1, c2, c3, c4 = st.columns([4, 2, 2, 1])
            c1.write(f"📅 {row['Fecha']} | {row['Concepto']}")
            c2.write(f"**${row['Monto']}**")
            if row['Estado'] == "PENDIENTE":
                if c3.button(f"{icn} CONFIRMAR", key=f"ok_{alias}_{row['row_idx']}"):
                    conectar_seguro().worksheet(hoja).update_cell(row['row_idx'], 5, est_ok); st.rerun()
            else:
                c3.write(f"✅ {row['Estado']}")
            if c4.button("🗑️", key=f"del_{alias}_{row['row_idx']}"):
                conectar_seguro().worksheet(hoja).delete_rows(row['row_idx']); st.rerun()

with tabs[0]: # VENTAS
    df_v_mes = df_v[df_v['Fecha'] >= primer_dia_mes] if not df_v.empty else pd.DataFrame()
    st.markdown(f"<div class='sum-box'>💰 TOTAL VENTAS MES: $ {df_v_mes['Monto'].sum() if not df_v_mes.empty else 0}</div>", unsafe_allow_html=True)
    with st.form("fv"):
        f_v = st.date_input("Fecha", hoy); m_v = st.number_input("Monto $", min_value=0.0)
        if st.form_submit_button("GUARDAR VENTA"):
            conectar_seguro().worksheet("Ventas").append_row([str(f_v), sede_act, "Venta Diaria", m_v, "PAGADO"])
            st.rerun()
    for _, row in df_v_mes.iterrows():
        c1, c2, c3 = st.columns([6, 2, 1])
        c1.write(f"📅 {row['Fecha']} | Venta Diaria"); c2.write(f"**${row['Monto']}**")
        if c3.button("🗑️", key=f"v_del_{row['row_idx']}"):
            conectar_seguro().worksheet("Ventas").delete_rows(row['row_idx']); st.rerun()

with tabs[1]: render_modulo("Fijos", "f", "Gasto Fijo", "💸", "PAGADO")
with tabs[3]: render_modulo("Cobros", "c", "Cuenta Cobro", "💰", "COBRADO")
