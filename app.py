...
# --- 1. CONEXIÓN BLINDADA (SOLUCIÓN AL ERROR DE PADDING) ---
def conectar():
    scope = ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive"]
    # Clave privada procesada para evitar errores en móviles
...
# --- 2. CONFIGURACIÓN ---
st.set_page_config(page_title="EMI MASTER V190", layout="wide")
hoy = date.today()
...
st.markdown("""
    ...
    .stButton>button { width: 100%; height: 32px; border-radius: 5px; font-weight: bold; }
...
with st.sidebar:
    ...
    st.info(f"💾 **Arrastre al {primer_dia}:** $ {arrastre}")
    v_mes = df_v[df_v['Fecha'] >= primer_dia]['Monto'].sum() if not df_v.empty else 0
    st.metric("BALANCE NETO TOTAL", f"$ {round(b_ini + c_ini + arrastre + v_mes, 2)}")

# --- 4. RENDERIZADO POR PESTAÑAS ---
tabs = st.tabs(["💰 VENTAS", "🏢 FIJOS", "🐜 HORMIGA", "🚛 PROVEEDORES", "📞 COBROS", "📊 REPORTES"])

def render_seccion(hoja, alias, label, icn_btn, est_ok):
    df_raw = leer_datos(hoja)
    ...
    if not df.empty:
        for _, row in df.iterrows():
            c1, c2, c3, c4, c5 = st.columns([3.5, 2, 1, 1, 1])
            c1.write(f"📅 {row['Fecha']} | {row['Concepto']}")
            c2.write(f"**${row['Monto']}**")
            if row['Estado'] == "PENDIENTE":
                if c3.button(icn_btn, key=f"ok_{alias}_{row['n_fila']}"):
                    conectar().worksheet(hoja).update_cell(row['n_fila'], 5, est_ok); st.rerun()
            else:
                if c3.button("🔄", key=f"rev_{alias}_{row['n_fila']}"):
                    conectar().worksheet(hoja).update_cell(row['n_fila'], 5, "PENDIENTE"); st.rerun()
            c4.button("📝", key=f"ed_{alias}_{row['n_fila']}")
            if c5.button("🗑️", key=f"del_{alias}_{row['n_fila']}"):
...
with tabs[0]: # VENTAS
    ...
    for _, row in v_mes.iterrows():
        ...
        c3.button("📝", key=f"ved_{row['n_fila']}")
        if c4.button("🗑️", key=f"vdel_{row['n_fila']}"):
            conectar().worksheet("Ventas").delete_rows(row['n_fila']); st.rerun()

with tabs[1]: render_seccion("Fijos", "f", "Gasto Fijo", "💸", "PAGADO")
with tabs[2]: render_seccion("Hormiga", "h", "Gasto Hormiga", "💸", "PAGADO")
with tabs[3]: render_seccion("Proveedores", "p", "Proveedor", "💸", "PAGADO")
with tabs[4]: render_seccion("Cobros", "c", "Cuenta Cobro", "💰", "COBRADO")
